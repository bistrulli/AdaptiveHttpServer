...
import three_tier_sys.Client;
import three_tier_sys.Tier1HTTPHandler;
import three_tier_sys.Tier2HTTPHandler;

public class three_tier_app {

	static String[] queues = new String[] { "think", "e1_bl", "e1_ex", "e2_bl", "e2_ex"};
	private static int initPop = -1;
	private static Boolean isEmu = false;

	public static void main(String[] args) {

		three_tier_app.getCliOptions(args);

		Jedis jedis = new Jedis("localhost");
		
		three_tier_app.resetState(jedis, three_tier_app.initPop);
		jedis.close();

		SimpleTask[] Sys = three_tier_app.genSystem(three_tier_app.initPop);
		three_tier_app.startSystem(Sys);
	}

	public static void resetState(Jedis jedis, int initDriver) {
		Transaction a = jedis.multi();
		for (int i = 0; i < three_tier_app.queues.length; i++) {
			if (i == 0)
				a.set(three_tier_app.queues[i], String.valueOf(initDriver));
			else
				a.set(three_tier_app.queues[i], String.valueOf(0));
		}
		a.exec();
		a.close();
	}
	...
	public static SimpleTask[] genSystem(int initDriver) {
		...
		HashMap<String, Class> t2Entries = new HashMap<>();
		HashMap<String, Long> t2Entries_stimes = new HashMap<>();
	...
	public static void getCliOptions(String[] args) {
		...
		while ((c = g.getopt()) != -1) {
			switch (c) {
			case 0:
				three_tier_app.initPop = Integer.valueOf(g.getOptarg());
				break;
			case 1:
				try {
					three_tier_app.isEmu =Integer.valueOf(g.getOptarg())>0?true: false;
